...
const tamilKeys = [
  ...
  // Uyir diacritics (vowel signs to combine with consonants)
  [
     '',   // base (அ sound, no mark)
  'ா',  // ஆ
  'ி',  // இ
  'ீ',  // ஈ
  'ு',  // உ
...
export default function SubmitRemedy() {
  ...
  // Helper: Tamil field wrapper with keyboard toggle
  const TamilField = ({ id, label, value, placeholder, rows = 3 }: { id: string, label: string, value: string, placeholder: string, rows?: number }) => (
    <div className="space-y-2 relative">
      ...
      {showKeyboard.field === id && (
        <VirtualKeyboard
          onInsert={handleKeyboardInsert}
          onClose={() => setShowKeyboard({ field: null })}
        />
      )}
    </div>
  );
        ...
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-8">
            ...
            <p className="text-muted-foreground">
              Share your traditional healing knowledge with the community
            </p>
          </div>
            ...
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        required
                        placeholder="e.g., Turmeric Milk for Cold"
                      />
                    </div>
                    
                    <TamilField
                      id="nameTa"
                      label="Remedy Name (Tamil)"
                      value={formData.nameTa}
                      placeholder="e.g., சளிக்கு மஞ்சள் பால்"
                    />
                  </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <Textarea
                        id="symptomsEn"
                        value={formData.symptomsEn}
                        onChange={(e) => handleChange('symptomsEn', e.target.value)}
                        required
                        placeholder="Common Cold, Cough, Sore Throat"
                        rows={3}
                      />
                    </div>
                    <TamilField
                      id="symptomsTa"
                      label="Symptoms (Tamil)"
                      value={formData.symptomsTa}
                      placeholder="சளி, இருமல், தொண்டை வலி"
                      rows={3}
                    />
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <Textarea
                        ...
                        onChange={(e) => handleChange('ingredientsEn', e.target.value)}
                        required
                        placeholder="1 cup milk, 1/2 tsp turmeric powder"
                        rows={4}
                      />
                    </div>
                    <TamilField
                      id="ingredientsTa"
                      label="Ingredients (Tamil)"
                      value={formData.ingredientsTa}
                      placeholder="1 கப் பால், 1/2 டீஸ்பூன் மஞ்சள் தூள்"
                      rows={4}
                    />
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <Textarea
                        ...
                        onChange={(e) => handleChange('preparationEn', e.target.value)}
                        required
                    ...
                    <TamilField
                      id="preparationTa"
                      label="Preparation (Tamil)"
                      value={formData.preparationTa}
                      placeholder="படிப்படியாக தயாரிப்பு முறை..."
                      rows={5}
                    />
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <Textarea
                        id="dosageEn"
                        value={formData.dosageEn}
                        onChange={(e) => handleChange('dosageEn', e.target.value)}
                        required
                        placeholder="Drink once daily before bedtime"
                        rows={3}
                      />
                    </div>
                    <TamilField
                      id="dosageTa"
                      label="Dosage (Tamil)"
                      value={formData.dosageTa}
                      placeholder="தினமும் இரவு படுக்கும் முன் ஒருமுறை"
                      rows={3}
                    />
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <Textarea
                        id="safetyTipsEn"
                        value={formData.safetyTipsEn}
                        onChange={(e) => handleChange('safetyTipsEn', e.target.value)}
                        required
                    ...
                    <TamilField
                      id="safetyTipsTa"
                      label="Safety Tips (Tamil)"
                      value={formData.safetyTipsTa}
                      placeholder="முக்கிய பாதுகாப்பு குறிப்புகள்..."
                      rows={4}
                    />
                  </div>
                </div>
                ...
                <div className="flex justify-end space-x-4 pt-6 border-t">
                  <Button type="button" variant="outline" onClick={() => navigate('/remedies')}>
                    {t('cancel')}
                  </Button>
                  <Button
                    type="submit"
                    disabled={isLoading}
                    className="bg-herbal hover:bg-herbal-dark transition-smooth min-w-32"
                  >
                    {isLoading ? (
                      <>
